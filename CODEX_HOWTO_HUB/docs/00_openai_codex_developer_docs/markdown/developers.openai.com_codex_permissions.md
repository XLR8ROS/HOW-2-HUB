# developers.openai.com_codex_permissions
...
# Permissions 
 ...
 Copy Page Beta. Permission profiles are under active development and may change.
...
```
 ...
 Path Meaning Scoped subpaths `:root` The filesystem root `.` only `:minimal` Platform and runtime paths needed by common tools `.` only `:workspace_roots` The current session’s workspace roots plus any enabled profile-defined workspace roots Yes `:tmpdir` The `$TMPDIR` location, when one is available `.` only `/absolute/path` A platform absolute path, such as `/path` on macOS/Linux/WSL or `C:\path` on native Windows Yes `~/path` A path under the current user’s home directory Yes 
