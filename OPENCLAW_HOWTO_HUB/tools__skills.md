...
For non-interactive runs, repeat `--skill ` for the exact skills to copy.
...
| -------------------- | ------------------------------------------- | --------------------------- |
| Per-agent            | `/skills`                        | Only that agent             |
| Project-agent        | `/.agents/skills`                | Only that workspace's agent |
...
Skill Workshop writes only to `/skills`, scans generated
...
| Action                                 | Command                                                |
| -------------------------------------- | ------------------------------------------------------ |
| Install a ClawHub skill into workspace | `openclaw skills install `                 |
...
| Install a skill for all local agents   | `openclaw skills install  --global`        |
| Update all workspace-installed skills  | `openclaw skills update --all`                         |
| Update a single shared managed skill   | `openclaw skills update  --global`         |
...
`skills///SKILL.md`, so related third-party skills can be
...
then falls back to the source directory or repository name. Use `--as ` to
...
`openclaw skills install ` or ClawHub install flow. It is off by default
...
`skills/` directory; the archive's internal folder name is ignored for the
...
`openclaw skills install ` remains only the install path; publishers
...
- `openclaw skills install ` is different — it downloads a ClawHub skill
...
### Optional frontmatter keys
  ...
  For tool dispatch, forwards the raw args string to the tool (no core parsing). The tool is invoked with `{ command: "", commandName: "", skillName: "" }`.
...
2. Applies `skills.entries..env` and `skills.entries..apiKey` to `process.env`.
...
`/skills/manager -> ~/Projects/manager/skills`. Managed
