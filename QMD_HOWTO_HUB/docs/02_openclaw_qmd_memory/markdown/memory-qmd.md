...
- 
 ...
 🇺🇸 English 🇺🇸 English ✓ 🇨🇳 简体中文 ✓ 🇨🇳 繁體中文 ✓ 🇯🇵 日本語 ✓ 🇪🇸 Español ✓ 🇧🇷 Português (BR) ✓ 🇰🇷 한국어 ✓ 🇩🇪 Deutsch ✓ 🇫🇷 Français ✓ 🇸🇦 العربية ✓ 🇮🇹 Italiano ✓ 🇻🇳 Tiếng Việt ✓ 🇳🇱 Nederlands ✓ 🇹🇷 Türkçe ✓ 🇺🇦 Українська ✓ 🇮🇩 Bahasa Indonesia ✓ 🇵🇱 Polski ✓ 🇮🇷 فارسی ✓ 🇹🇭 ไทย ✓ 
 Search... ⌘K 
 GitHub Releases Discord 
 Menu 
 
 Get started Install Channels Agents Capabilities ClawHub Models Platforms Gateway & Ops Reference Help 
...
## Fundamentals
 Gateway architecture Agent runtime Agent loop Agent runtimes System prompt Context Context engine Agent workspace SOUL.md personality guide OAuth Agent bootstrapping Experimental features QA overview Personal agent benchmark pack Matrix QA 
## Sessions and memory
 Session management Channel docking Session pruning Session tools 
## Memory
 Memory overview Builtin memory engine QMD memory engine Honcho memory Memory search Active memory Inferred commitments Dreaming Compaction 
## Multi-agent
 Multi-agent routing Parallel specialist lanes Presence Delegate architecture 
## Messages and delivery
 Messages Message lifecycle refactor Streaming and chunking Progress drafts Retry policy Command queue Steering queue 
 
 
 
 
 Agents / Sessions and memory / QMD memory engine 
 Sessions and memory

# QMD memory engine

 Copy page Edit source 
...
### Enable
 ...
 json5 Copy code { memory : { backend : "qmd" , }, } OpenClaw creates a self-contained QMD home under
...
same-source collections with one command:
 ...
 bash Copy code qmd search "router notes" --json -n 10 -c memory-root-main -c memory-dir-main This avoids starting one QMD subprocess for every durable-memory collection.
...
separately before merging and deduplicating results.
 ...
 bash Copy code qmd -- help | grep -i collection Current QMD help says collection filters can target one or more collections.
...
process, so you can tune QMD globally without adding new OpenClaw config:
 ...
 bash Copy code export QMD_EMBED_MODEL= "hf:Qwen/Qwen3-Embedding-0.6B-GGUF/Qwen3-Embedding-0.6B-Q8_0.gguf" export QMD_RERANK_MODEL= "/absolute/path/to/reranker.gguf" export QMD_GENERATE_MODEL= "/absolute/path/to/generator.gguf" After changing the embedding model, rerun embeddings so the index matches the
new vector space.
...
## Indexing extra paths
 ...
 json5 Copy code { memory : { backend : "qmd" , qmd : { paths : [{ name : "docs" , path : "~/notes" , pattern : "**/*.md" }], }, }, } Snippets from extra paths appear as qmd/<collection>/<relative-path> in
...
## Indexing session transcripts
 ...
 json5 Copy code { memory : { backend : "qmd" , qmd : { sessions : { enabled : true }, }, }, } Transcripts are exported as sanitized User/Assistant turns into a dedicated QMD
...
(not groups). Configure memory.qmd.scope to change this:
 ...
 json5 Copy code { memory : { qmd : { scope : { default : "deny" , rules : [{ action : "allow" , match : { chatType : "direct" } }], }, }, }, } When scope denies a search, OpenClaw logs a warning with the derived channel and
...
interactive shell. Pin the binary explicitly:
 ...
 json5 Copy code { memory : { backend : "qmd" , qmd : { command : "/absolute/path/to/qmd" , }, }, } Use command -v qmd in the environment where QMD is installed, then recheck
...
- Honcho memory 
 ...
 Previous Builtin memory engine Next Honcho memory 
 
 
## On this page
 What it adds over builtin Getting started Prerequisites Enable How the sidecar works Search performance and compatibility Model overrides Indexing extra paths Indexing session transcripts Search scope Citations When to use Troubleshooting Configuration Related 
 
 
 Close install telegram gateway plugins 
